...
{
        int rc;
        struct iovec msgvec[2] = { 0 };
        struct msghdr msghdr = { 0 };

        memset(&msgvec[0], 0, sizeof(struct iovec));
        memset(&msgvec[1], 0, sizeof(struct iovec));
                                ...
                                struct sockaddr_ll *so_addr)
{
        ...
        struct mip_msg *msg = calloc(1, sizeof(struct mip_msg) + ETH_PAYLOAD);
